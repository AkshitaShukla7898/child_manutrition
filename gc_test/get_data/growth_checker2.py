...
def checker(dic,dict):
   ...
   # gender = input("Gender of child -> ").lower()
    last_checked = str(dic.get('age_when_last_weighed'))
    weight_last = float(dic.get('previous_weight'))
   # age = str(input("Child age is -> "))
   # weight = float(input("Child weight is -> "))
    birth_weight = float(dic.get('weight_at_birth'))
    ...
    print("*******")
    weight_gain = "{:.2f}".format(1000 * (weight - weight_last))
    weight_gain_per_gram = "{:.2f}".format(1000 * (weight - weight_last) / ((int(age) - int(last_checked)) * 7))
    st='Weight gain in this time = ' + str(weight_gain)+ ' grams '+ '<br>'+ 'Weight gain per day during this time = ' + str(weight_gain_per_gram) +' grams'

    x = 0
    print("********")
    if gender == "girl":
        for i in range(int(age) - int(last_checked)):
            new_age = int(last_checked) + i + 1

            sta= 'A healthy born child would have grown'+ str(data_girl[str(int(last_checked) + i)]['gram/day'])+ ' grams from age of ' + str(int(last_checked) + i) + ' week to age of '+ str(new_age) +' week '

            x += data_girl[str(int(last_checked) + i)]['gram/day'] * 7

    elif gender == "boy":
        for i in range(int(age) - int(last_checked)):
            new_age = int(last_checked) + i + 1
            print(
                f"A healthy born child would have grown {data_boy[str(int(last_checked) + i)]['gram/day']} grams from age of {int(last_checked) + i} week to "
                f"age of {new_age} week ")

            sta = 'A healthy born child would have grown' + str(data_boy[str(int(last_checked) + i)]['gram/day']) + ' grams from age of ' + str(int(last_checked) + i) + ' week to age of ' + str(new_age) + ' week '

            x += data_boy[str(int(last_checked) + i)]['gram/day'] * 7

    print("********")
    stb='Average weight gain per day in this period for a healthy born child would have been ' + str(x / ((int(age) - int(last_checked)) * 7)) + ' grams/day'

    print("******")
    weight_gain_from_birth_per_gram = "{:.2f}".format(1000 * (weight - birth_weight) / (int(age) * 7))
    weight_gain_from_birth = weight_gain = "{:.2f}".format(1000 * (weight - birth_weight))

    stc= 'Child has gain' + str(weight_gain_from_birth) + ' grams from birth on an average of '+ str(weight_gain_from_birth_per_gram) + ' grams per day'
    ...
    oo = st + '<br>' + sta + '<br>' + stb  + '<br>' + stc
    oop = op + oo
    return oop

    # weight gain from birth ------ done
